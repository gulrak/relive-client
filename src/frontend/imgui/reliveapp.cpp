...
{
    auto hash = relive::hash(str);//hashFNV1a(str.c_str());
    float r = 0, g = 0, b = 0;
...
void ReLiveApp::handleInput(ImGuiIO& io)
{
    ...
    if(_lateSetup) {
        ...
        if (savedPosition.empty() || !openURL(savedPosition, false)) {
            for (const auto& station : _stations) {
...
void ReLiveApp::renderPlayBar(ImVec2 size)
{
    ...
    drawList->AddRectFilled(ImVec2(pos.x, pos.y + PLAY_BAR_HEIGHT+0.5), ImVec2(pos.x + size.x * _player.receiveBufferQuote(), pos.y + PLAY_BAR_HEIGHT + 4), IM_COL32(20,80,20,255));
    drawList->AddRectFilled(ImVec2(pos.x, pos.y + PLAY_BAR_HEIGHT+4.5), ImVec2(pos.x + size.x * _player.decodeBufferQuote(), pos.y + PLAY_BAR_HEIGHT + 8), IM_COL32(20,60,60,255));
